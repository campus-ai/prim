...
export function parseWorkosConnectConfiguration(value: unknown): WorkosConnectConfiguration {
  const record = recordFrom(value, "Connect discovery");
  const expectedKeys = ["protocol_version", "issuer", "client_id", "default_scopes"];
  if (
    record.protocol_version !== 1 ||
    Object.keys(record).length !== expectedKeys.length ||
    expectedKeys.some((key) => !Object.hasOwn(record, key)) ||
    !Array.isArray(record.default_scopes) ||
    record.default_scopes.length !== WORKOS_CONNECT_DEFAULT_SCOPES.length ||
    record.default_scopes.some((scope, index) => scope !== WORKOS_CONNECT_DEFAULT_SCOPES[index])
  ) {
  ...
  const clientId = exactString(record.client_id, "client_id", MAX_CLIENT_ID_LENGTH);
