 ...
 */
export async function drainFlushingPath(flushingPath: string, client: CliClient): Promise<number> {
...
async function drainPath(path: string, client: CliClient): Promise<number> {
...
async function flushOnce(): Promise<{
  ...
  if (!inspection.client) {
    return { flushed: 0, retained: inspection.retainedBuckets };
  }
  ...
  for (const { bucket, path } of liveBuckets) {
    ...
    try {
      total += await drainPath(path, client);
    } catch (err) {
  ...
  return inspection.retainedBuckets.length > 0
    ? { flushed: total, retained: inspection.retainedBuckets }
    : { flushed: total };
}
