...
describe("WorkOS user API-key mint", () => {
  ...
  it("binds an exact requested expiration to the response", async () => {
  ...
  it("accepts no response and never retries an uncertain transport failure", async () => {
    const { dependencies, post, stdout } = harness();
    ...
    expect(post).toHaveBeenCalledOnce();
    expect(machine(stdout)).toMatchObject({ code: "transport_error" });
