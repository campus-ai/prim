...
// Claim only exact stable or legacy commands for this config root. A shape or
// suffix match could delete a user's unrelated shim under a different
...
export function hookRuntimeResolutions(hooks: HooksMap): HookCommandResolution[] {
  ...
  for (const entries of Object.values(hooks)) {
    for (const entry of entries) {
      for (const bin of PRIM_BINS) {
        if (entry.command === legacyCommandFor(bin)) resolutions.push({ kind: "legacy_path" });
      }
    }
  }
  return resolutions;
