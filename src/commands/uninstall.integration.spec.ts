...
describe("prim uninstall filesystem composition", () => {
  it("contracts recognized runtimes while preserving credentials, journals, bindings, and skills", async () => {
    ...
    const bindingPath = join(root, "project", ".prim", "workspace.json");
    const skillPath = join(homeDir, ".codex", "AGENTS.md");
    ...
    writeFileSync(bindingPath, '{"orgId":"org_1"}\n');
    writeFileSync(
      skillPath,
      "<!-- BEGIN PRIM SKILL v1 -->\nguidance\n<!-- END PRIM SKILL v1 -->\n",
    );
    ...
    expect(readFileSync(bindingPath, "utf8")).toBe('{"orgId":"org_1"}\n');
