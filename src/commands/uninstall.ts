...
export type UninstallCommandDependencies = {
  ...
  removeRuntimes?: () => Promise<{ hookRuntimeChanged: boolean; daemonRuntimeChanged: boolean }>;
  ...
  program
    ...
    .action(async () => {
      ...
      if (failed.length > 0) {
        ...
        try {
          const removed = await removeRuntimes();
          runtime = { status: "removed", ...removed };
          note("uninstall · recognized runtime state removed");
        } catch (error) {
          const detail = error instanceof Error ? error.message : String(error);
          failed.push("runtime");
          runtime = { status: "retained", reason: detail };
          note(`uninstall · runtime retained: ${detail}`);
        }
      }
