...
- **warn / additional context** -- the edit proceeds, but a relevant prior decision is surfaced. Read it. Codex cannot pause mid-tool, so a would-be `ask` arrives as a **deny** carrying the reconcile directive. Hermes also has no soft-confirm tier, so a would-be `ask` arrives as a **deny** carrying the same directive: reconcile and retry, or set `PRIM_HOOK_MODE=warn` to downgrade it to context-only.
